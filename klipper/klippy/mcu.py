...
class MCU:
    ...
    def _connect(self):
        ...
        if not config_params['is_config']:
            ...
            start_reason = self._printer.get_start_args().get("start_reason")
            if start_reason == 'firmware_restart':
                # raise_msg = ("[MKS_DEBUG]\tFailed automated reset of MCU '%s', exce firmware_restart"
                #              % (self._name,))
                # logging.info(raise_msg)
                # self._printer.lookup_object('gcode').request_restart('firmware_restart')
